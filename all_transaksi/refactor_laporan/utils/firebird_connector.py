    ...
    def _get_column_positions(self, separator_line):
        ...
        for i, char in enumerate(separator_line):
            ...
            if is_separator and not in_column:
                start = i
                in_column = True
            # If we find a non-separator char and we are in a column, end's column
            elif not is_separator and in_column:
                positions.append((start, i))
                in_column = False
                start = None

        # If line ends with a separator character, add to final column
        ...
        if not positions:
            ...
            # Add last word if it goes to end of line
