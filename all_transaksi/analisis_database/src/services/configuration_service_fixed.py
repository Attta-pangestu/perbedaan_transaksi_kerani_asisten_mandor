...
class ConfigurationServiceFixed:
    """
    ...
    def get_configuration_status(self) -> Dict[str, Any]:
        ...
        """
        try:
            config = self._estates_cache if self._estates_cache else {}
            validation_results = self.validate_all_estates()

            status = {
                'config_file_path': self.config_repo.config_file_path,
                'config_file_exists': os.path.exists(self.config_repo.config_file_path),
                'cached_estates': len(config),
                'cache_timestamp': self._last_cache_update.isoformat() if self._last_cache_update else None,
                'total_estates': len(validation_results),
                'valid_estates': sum(1 for r in validation_results.values() if r['valid']),
                'estates_with_paths': sum(1 for r in validation_results.values() if r.get('database_path') and r['database_path'].strip()),
                'estates_with_existing_files': sum(1 for r in validation_results.values() if r.get('database_path') and os.path.exists(r['database_path'])),
                ...
                'validation_passed': all(r.get('valid', False) for r in validation_results.values()),
    ...
    def get_configuration_summary(self) -> str:
        ...
        try:
            ...
            summary.append(f"  Config file: {status['config_file_path']}")
            summary.append(f"  File exists: {status['config_file_exists']}")
            summary.append(f"  Cached estates: {status['cached_estates']}")
            summary.append(f"  Total estates: {status['total_estates']}")
            summary.append(f"  Valid estates: {status['valid_estates']}")
            summary.append(f"  Estates with paths: {status['estates_with_paths']}")
            summary.append(f"  Estates with existing files: {status['estates_with_existing_files']}")

            if status.get('total_issues', 0) > 0:
                summary.append(f"  Total issues: {status['total_issues']}")

            if status.get('total_warnings', 0) > 0:
                summary.append(f"  Total warnings: {status['total_warnings']}")

            summary.append(f"  Validation passed: {status['validation_passed']}")
            summary.append(f"  Last updated: {status['last_updated']}")

            return '\n'.join(summary)

        except Exception as e:
            return f"Error getting configuration summary: {e}"