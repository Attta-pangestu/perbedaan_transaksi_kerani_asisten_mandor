"""
Firebird Database Connector Module
Provides database connectivity for Firebird databases using ISQL.
"""
...
class FirebirdConnector:
    ...
    def _parse_isql_output(self, output_text: str, as_dict: bool = True) -> List[Dict[str, Any]]:
        """
        ...
        if has_separator_line and possible_header_line and header_positions:
            ...
            for line in data_lines:
                ...
                if any(v.strip() for v in row.values()):
                    # Additional validation: skip rows that contain header names as values
                    # This prevents duplicate header lines from being treated as data
                    is_header_row = False
                    for col_name, value in row.items():
                        if value == col_name or value in ['RECORDTAG', 'TRANSTYPE', 'XX']:
                            is_header_row = True
                            break
    ...
    def _get_column_positions(self, separator_line: str) -> List[tuple]:
        """
        Mendapatkan posisi kolom dari baris separator ISQL

        ISQL format uses ===== to indicate column boundaries.
        Each group of ===== characters represents a column.
        ...
        # Find all groups of = characters - each group represents a column
        i = 0
        n = len(separator_line)

        while i < n:
            # Skip non-separator characters
            if separator_line[i] not in '=-':
                i += 1
                continue
            ...
            # Find end of this separator group
            while i < n and separator_line[i] in '=-':
                i += 1

            # End of separator group is the end of the column
            end = i

            # Only add if we have a meaningful column (at least 2 chars)
            if end - start >= 2:
                positions.append((start, end))

        # If we didn't find proper column separators, try space-based detection
        if not positions:
            print("No proper column separators found, trying space-based detection...")

            # Look for transitions from spaces to non-spaces and back
            in_column = False
            start = None

            for i, char in enumerate(separator_line):
                if char in '=-' and not in_column:
                    # Start of a column
                    start = i
                    in_column = True
                elif char not in '=- \t' and in_column:
                    # End of column when we hit a non-separator that's not whitespace
                    # But we might be in the header content, so continue
                    continue
                elif (char == ' ' or char == '\t') and in_column:
                    # Space might indicate end of column boundary
                    pass

            # For ISQL format, a better approach is to look for the actual column boundaries
            # by finding where the separator groups are separated by spaces
            if not positions:
                # Split by spaces and treat each group of separators as a column
                parts = separator_line.replace('\t', ' ').split()
                current_pos = 0

                for part in parts:
                    if '=' in part or '-' in part:
                        # Find this part in the original string
                        found_pos = separator_line.find(part, current_pos)
                        if found_pos != -1:
                            positions.append((found_pos, found_pos + len(part)))
    ...
    def get_example_query(self, table_name: Optional[str] = None) -> str:
    ...
    def to_pandas(self, result_data: List[Dict[str, Any]]) -> pd.DataFrame:
        """
...
class ConnectionFactory:
    ...
    @staticmethod
    def create_connection(db_path: str, username: str = 'sysdba',
                         password: str = 'masterkey',
                         use_localhost: bool = False) -> FirebirdConnector:
        """
        ...
        :return: FirebirdConnector instance
        """
    ...
    @staticmethod
    def test_connection(db_path: str) -> bool:
        """
        ...
        """
        try:
            connector = ConnectionFactory.create_connection(db_path)
